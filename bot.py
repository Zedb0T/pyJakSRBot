...
class CategorySelectionView(discord.ui.View):
    def __init__(self, categories, interaction, bot, game_id):
        ...
        self.add_item(SubmitButton(bot, game_id))
...
class CategorySelectionDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        # This callback is for the dropdown
        pass

class SubmitButton(discord.ui.Button):
    def __init__(self, bot, game_id):
        super().__init__(label="Submit", style=discord.ButtonStyle.primary, custom_id="submit_categories")
        self.bot = bot
        self.game_id = game_id

    async def callback(self, interaction: discord.Interaction):
        print("SubmitButton callback triggered")

        if interaction.custom_id != "submit_categories":
            print("Unexpected custom_id")
            return

        # Retrieve selected values from the dropdown
        selected_values = []
        for item in self.view.children:
            if isinstance(item, CategorySelectionDropdown):
        ...
        try:
            ...
            print(f"Selected categories saved to {file_path}")
            await interaction.response.send_message(f"Selected categories saved to {file_path}", ephemeral=True)
        except Exception as e:
            print(f"Error saving file: {e}")
            await interaction.response.send_message("There was an error saving the file.", ephemeral=True)
